...
static int[] SCALING_FACTORS = {100, 255, 100, #FFFFFF, 1000, 255, 25, 255, 255, 255}; // The maximum value that each dimension can display

int bckgndColor = 155; // Background color
...
void setTimeDimSpeed(int whichDim, int whatSpeed) {
  if (whichDim == X_DIM) {
    ...
    for (int i = 0; i < 1000; i += whatSpeed) { // Populate the list
      zTimeDim.add(-i); // Remember that z going into the screen is negative
    }
  } 
}
...
class Trace {
  ...
  // Constructor
  Trace (int[] tempWhichMeasurements, int tempTraceLength, int tempColorFormat) {
  ...
  void drawTrace() {
    for (int i = 0; i < trace.size() - 1; i++) { // Loop through all of the points in the trace
      ...
      if (trace.get(i).getTimeDimension() == X_DIM) { // Using 'time' for the x-axis
      ...
      } else if (trace.get(i).getTimeDimension() == Y_DIM) { // Using 'time' for the y-axis
      ...
      } else if (trace.get(i).getTimeDimension() == Z_DIM) { // Using 'time' for the z-axis
        println("Z Time"); // DEBUG
        line(trace.get(i).getDimension(X_DIM), trace.get(i).getDimension(Y_DIM), yTimeDim.get(i), trace.get(i+1).getDimension(X_DIM), trace.get(i+1).getDimension(Y_DIM), yTimeDim.get(i+1));
...
class DataPoint { // This is distinct from the Point class
  int[] values; // 
  int timeAxis = -1; // Denotes which, if any, of the axes are using 'time.' -1 means none.
  ...
  // Returns which, if any, of the dimensions, is using 'time'
