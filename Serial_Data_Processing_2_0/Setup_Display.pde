...
public void addTrace(int theValue) {
  if ( frameCount > 1) { // Quick fix, explained elsewhere
    ...
    for (int i = 0; i < dimensionOptions.length; i++) {
      ...
      // Check if there is a scrollable list for the dimension we're looking at
    ...
    traces.add(new Trace(tempWhichMeasurements, defaultTraceLength, tempColorFormat)); // Actually creating the new trace, (int[] whichMeasurements, int length)
    println("Trace added.\n----------------------------------------------------------------------------------------------\n\n"); // Debug
  }
}
